...
const configsWithMixins = {
  configMixinDefault: {
    ...
    file: path.resolve(__dirname, 'config-mixin/config-mixin.default.json'),
  },
  configMixinProductionOne: {
    config: configMixinProductionOne,
    file: path.resolve(__dirname, 'config-mixin/config-mixin.production-one.json'),
  },
  configMixinProductionTwo: {
    config: configMixinProductionTwo,
    file: path.resolve(__dirname, 'config-mixin/config-mixin.production-two.json'),
  },
  ...
  configMixinDevelopmentOne: {
    ...
    file: path.resolve(__dirname, 'config-mixin/config-mixin.development-one.json'),
  },
  configMixinDevelopmentTwo: {
    config: configMixinDevelopmentTwo,
    file: path.resolve(__dirname, 'config-mixin/config-mixin.development-two.json'),
  },
...
function updateConfig(config, update) {
  ...
  if (currConfig) {
    fs.writeFileSync(currConfig.file, JSON.stringify(_.merge({}, currConfig.config, update), null, 4));
  } else {
