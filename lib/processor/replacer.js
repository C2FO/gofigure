...
const performReplace = (configValue, matchedReplacement, replacement) => {
    return configValue.replace(matchedReplacement, replacement);
};
...
const doReplace = (any, envVars) => {
    ...
    if (_.isPlainObject(any)) {
        ...
        const flattenedReplaced = Object.keys(flattened).reduce((replaced, key) => {
            ...
            replaced[key] = doReplace(val, envVars);
            return replaced;
        }, {});
