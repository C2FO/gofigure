...
const _emitChanges = (config, deletedKeys, changedKeyValues, eventEmitter) => {
  const changedObjects = changedKeyValues
    .map((kv) => kv.key)
    .reduce((changed, key) => {
      // split the key
  ...
  const allChanges = _.sortBy(_.uniq([...changedKeyValues, ...changedObjects], 'key'), 'key');
  // emit the events
...
const merger = (oldConfig, newConfig, eventEmitter) =>
  _merge('', _.cloneDeep(oldConfig), _.cloneDeep(newConfig), eventEmitter);
