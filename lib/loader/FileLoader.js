...
class FileLoader extends PatternEventEmitter {
  ...
  __getConfigOrReadAsync() {
    return FileLoader.readFileAsync(this.file).then((fileConfig) => this.__processConfigAsync(fileConfig));
  }

  __processConfig(config) {
    if (Array.isArray(config.mixins)) {
      const mixinPaths = config.mixins.map((ep) => this.__addToMixins(path.resolve(path.dirname(this.file), ep)));
  ...
  __processConfigAsync(config) {
    if (Array.isArray(config.mixins)) {
      const mixinPaths = config.mixins.map((ep) => this.__addToMixins(path.resolve(path.dirname(this.file), ep)));
      return Promise.all(mixinPaths.map((mp) => FileLoader.readFileAsync(mp))).then((mixinConfigs) => {
  ...
  __addWatcher(filePath) {
    if (!this.fileWatchers.has(filePath)) {
      this.fileWatchers.set(
        filePath,
        fs.watch(filePath, (event) => this.watchHandler(event)),
      );
    }
  }
